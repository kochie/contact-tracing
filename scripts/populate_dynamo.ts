...
const locations = new Map<
  number,
  { latitude: number; longitude: number; location_id: number }
>();
l.forEach((lo) => {
        ...
        [checkinTable]: items.map((item) => ({
          PutRequest: {
            Item: {
              ...
              latitude: {
                S: locations.get(item.location_id)?.latitude.toString() || "",
              },
              longitude: {
                S: locations.get(item.location_id)?.longitude.toString() || "",
              },
            },
          },
        })),
